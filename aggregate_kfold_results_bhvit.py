...
def main():
    ...
    if class_source is None:
        raise SystemExit(
            f"No splits found under {first_fold_dir} (looked for val/holdout_val/test)."
        )
    ...
    prob_stack = {
        s: [] for s in common_splits if s != "val"
    }  # ensemble only on holdout_val/test typically
    ...
    for idx, (run_dir, fold_name) in enumerate(mappings):
        ...
        for split in common_splits:
            dl, _ = build_eval_loader(
                fold_root / split, train_args, args.batch_size, args.num_workers
            )
            y_true, y_pred, y_prob = predict(model, dl, device)

            # Save predictions
            save_json(
                out_dir / "predictions" / fold_name / f"{split}_preds.json",
                {
            ...
            tp.save_confusion_and_report(
                model, dl, device, normalize="true", file_prefix=split
            )

            # Acc for quick glance
            fold_metrics[f"{split}_acc"] = (
                float((y_pred == y_true).mean()) if len(y_true) else float("nan")
            )
    ...
    for split in prob_stack.keys():
        ...
        # Save preds
        save_json(
            out_dir / "predictions" / f"{split}_ensemble.json",
            {
                "y_true": y_true.tolist(),
                "y_pred": preds_ens.tolist(),
                "y_prob": probs_mean.tolist(),
            },
        )
        ...
        try:
            ...
            def plot_confusion(cm_arr, cname, out_png: Path, normalize=True):
                ...
                ax.set(
                    xticks=np.arange(len(cname)),
                    yticks=np.arange(len(cname)),
                    xticklabels=cname,
                    yticklabels=cname,
                    ylabel="True",
                    xlabel="Predicted",
            ...
            plot_confusion(
                cm, classes, out_dir / "figures" / f"{split}_confusion_ens.png"
            )
            ...
            for i in range(len(classes)):
                ...
                neg = (1 - Y[:, i]).sum()
                if pos > 0 and neg > 0:
                    fpr[i], tpr[i], _ = roc_curve(Y[:, i], probs_mean[:, i])
                    valid.append(i)
            if valid:
                ...
                plt.xlabel("FPR")
                plt.ylabel("TPR")
                plt.title(f"ROC ({split} ensemble)")
                plt.legend(loc="lower right")
                fig.tight_layout()
                fig.savefig(out_dir / "figures" / f"{split}_roc_ens.png")
                plt.close(fig)
